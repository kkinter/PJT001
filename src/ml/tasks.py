...
def batch_user_prediction_task(start_page=0, offset=250, max_pages=1000):
    ...
    recent_user_ids = profile_utils.get_recent_users()[10:]
...
@shared_task
def batch_update_user_predictions_task(user_ids=1, start_page=0, 
                                       offset=250, max_pages=100_000):
    model = ml_utils.load_model()
    end_page = start_page + offset
    recent_user_ids = [user_ids]
    ...
    for movie_id in movie_ids:
        for u in recent_user_ids:
            ...
            print(u, movie_id, pred)
    if end_page < max_pages:
        return batch_update_user_predictions_task(start_page=end_page-1)
